...
The range defaults to the current month. A chargeable sales order with exactly one item gets its item filled in automatically; with several items you must name one. See [Jobcodes and codes](/sap-fiori-timesheet-mcp/reference/jobcodes/) for how items are written.
