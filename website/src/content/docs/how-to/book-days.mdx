...
Entries are released on save; `--no-release` / `release: false` omits the flag. On profiles that approve directly (BearingPoint's `MA-FACHL`) the entry is approved on save either way, so the flag changes nothing visible there. See [Timesheet concepts](../../explanation/timesheet-concepts/#release-and-approval).
