...
Only one process can open the profile at a time. Wait for the other CLI command or MCP host to finish, or give this instance its own profile with `XFLOW_PROFILE_DIR` (it needs its own first sign-in). See [Run several instances at once](../sign-in/#run-several-instances-at-once).
...
<Aside type="note">
Exit codes and file locations are listed in [Files and exit codes](../../reference/files-and-exit-codes/).
</Aside>
