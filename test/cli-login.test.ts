...
describe("xflow-timesheet login (CLI)", () => {
  it("logs in with credentials from env and stores the session", async () => {
    ...
    const code = await runCli(["login"], {
      env: { XFLOW_LAUNCHPAD_URL: idp.launchpadUrl, XFLOW_SESSION_FILE: sessionFile, XFLOW_EMAIL: "arno@example.com", XFLOW_PASSWORD: "s3cret" },
      ...t,
    });
    expect(code).toBe(0);
  ...
  it("prompts for email and password when not provided", async () => {
    ...
    const code = await runCli(["login"], {
      env: { XFLOW_LAUNCHPAD_URL: idp.launchpadUrl, XFLOW_SESSION_FILE: join(dir, "s.json") },
      ...t,
    });
    expect(code).toBe(0);
  ...
  it("reports session status and logs out", async () => {
    const dir = mkdtempSync(join(tmpdir(), "xflow-cli-"));
    const env = { XFLOW_LAUNCHPAD_URL: idp.launchpadUrl, XFLOW_SESSION_FILE: join(dir, "s.json") };
    let t = io([]);
